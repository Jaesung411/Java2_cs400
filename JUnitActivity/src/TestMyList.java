...
public class TestMyList {
    ...
    public void testRemove(){
	    ...
	    for(int j = 10; j>0; j--){
		    _instance.remove(j-1);
		    assertEquals(j-1, _instance.size());
	    }
    }

    //test if the size is zero after clear method works.y
    @Test
    ...
    //test if all subsequent elements are shift to the left when removing.
    @Test
    public void testIndexRemove(){
	    ...
	    for(int j = 10; j<20; j++){;
		    assertEquals(j, _instance.remove(0));
	    }
    }
