...
public class DijkstraGraph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
    	...
    	if(!containsVertex(start) || !containsVertex(end)) {
    		throw new NoSuchElementException("There is no such element in the graph");
    	}
        ...
        while(shortest.end != vertices.get(end)){
        	...
        	if(num == 0) {
        		...
        		shortest = PQ.poll();
        	} else { //else edge is leaving -> check existing data by looking up dataSequence
            	for(int i=0; i<num; i++) {
            		int dataSize = shortest.dataSequence.size();
            		int countD = 0; //count for condition that there isn't existing data
            		for(int j=0; j<dataSize; j++) {
            			if(vEdges.get(i).target.data != shortest.dataSequence.get(j)) {
            				countD++;
            			}
            		}
    ...
    /**
     * Connect two vertices both ways by calling the insertEdge method twice, alternating source
     * and target to create two edges.
     *
     * @param placeA Object representing first vertex
     * @param placeB Object representing second vertex
     * @param weight Weight of the two new edges
     * @return true if the vertices could be connected or edge weight updated, false if an edge
     * with the same weight and vertices connected was in the graph
     * the same weight was already in the graph with the graph
     */
    public boolean connectBothWays(T placeA, T placeB, int weight) {

    	boolean firstInsert = insertEdge(placeA, placeB, weight);
    	boolean secondInsert = insertEdge(placeB, placeA, weight);

    	if(firstInsert && secondInsert) {
    		return true;
    	}
    	else {
    		System.out.println("Can't connect both ways");
    		return false;
    	}
    }

    /**
     * Calculates the distance between two locations using their xyz coordinates. Employs the
     * pythagorean theorem.
     *
     * @param placeA first location
     * @param placeB second location
     * @return distance between locations
     */
    public int calculateEdgeWeight(IPlace placeA, IPlace placeB) {

    	//exception if placeA and placeB is equal to each other
    	if(placeA.equals(placeB)) {
    		throw new IllegalArgumentException("two place is the same");
    	}

    	//set coordinates
    	double xA = placeA.getX();
    	double xB = placeB.getX();
    	double yA = placeA.getY();
    	double yB = placeB.getY();
    	double zA = placeA.getZ();
    	double zB = placeB.getZ();

    	//employ the pythagorean theorem
    	double powX = Math.pow(xA-xB, 2);
    	double powY = Math.pow(yA-yB, 2);
    	double powZ = Math.pow(zA-zB, 2);
    	double edgeWeight = Math.sqrt(powX+powY+powZ);

    	return (int)Math.round(edgeWeight);
    }
