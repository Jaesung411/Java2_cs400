...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
        	...
        	List<T> extendDataSequence = new LinkedList<>();
        	this.start = copyPath.start;
        	this.distance = copyPath.distance + extendBy.weight;
        	this.end = extendBy.target;
        	for(T d:copyPath.dataSequence) {
        		extendDataSequence.add(d);
        	}
        	this.dataSequence = extendDataSequence;
  ...
  protected Path dijkstrasShortestPath(T start, T end) {   
		...
		//NoSuchElementException when no vertex containing start or end can be found
	  	if(!containsVertex(start)||!containsVertex(end)) throw new NoSuchElementException();
	  	
	  	Path startingPath = new Path(vertices.get(start)); // a given starting point
	  	Vertex current = vertices.get(start); //vertex in progress
	  	PriorityQueue<Path> pq = new PriorityQueue<Path>();//the priority queue saving all sorted paths
	  	LinkedList<Vertex> passedVertices = new LinkedList<Vertex>(); // passed vertices 
		LinkedList<Vertex> possibleVertices = new LinkedList<Vertex>(); // vertices that can go from vertex in progress
	  	
		//set the starting path
	  	pq.add(startingPath);
	  	passedVertices.add(current);
	  	
	  	//keep searching until finding the end point
	  	while(!pq.peek().end.data.equals(end)) {
	  		
	  		for(Edge e : current.edgesLeaving) {
	  			//collect vertices that go possible for checking if there is any path from start to end can be found
	  			possibleVertices.add(e.target);
	  			
	  			//add the path if the vertex was not passed
	  			if(!passedVertices.contains(e.target)) {
	  				pq.add(new Path(pq.peek(),e));
	  			}
	  		}
	  		//NoSuchElementException when the node in progress can go to only passed vertices 
	  		if(passedVertices.containsAll(possibleVertices)) throw new NoSuchElementException();
	  		
	  		//remove the past shortest path
	  		pq.remove();
	  		
	  		//update the node in progress and add it in the passed vertices
	  		current = pq.peek().end;
	  		passedVertices.add(current);
	  	}
	  	return pq.peek(); // TODO: plement this method in Step 7.
    }
	...
	public boolean connectBothWays(T placeA, T placeB, int weight) {
		...
		this.insertEdge(placeA, placeB, weight);
		return false;
	}
